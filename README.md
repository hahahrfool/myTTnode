...
另外就是每次重启间隔时间长的话还是会重新绑定，并没有我所推荐的docker版本的GitHub主所写的hostname不变就会不变的情况。

另外，关于软路由推荐，因为我是要一机多用，跑脚本，passwall, 甜糖，HA等，因此N1应该不考虑，所以个人认为我的选择还算合理。如果要求不高的小伙伴可以选择降配，但注意硬盘64g, 内存4g, 不能再降。
